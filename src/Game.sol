...
contract Game is Ownable {
  ...
  function makeNewBoss(string calldata _name, uint256 _total_power) external onlyOwner {
    current_boss = Character({name: _name, power_left: _total_power, experience: 0, created: true, dead: false});
  }

  function makeNewBoss(string calldata _name) external onlyOwner {
    current_boss = Character({name: _name, power_left: _getRandomPower(), experience: 0, created: true, dead: false});
  }
  ...
  function createNewCharacter() external {
    ...
    Character memory _newChar =
      Character({name: characterNames[_nameIndex], power_left: _power, experience: 0, created: true, dead: false});
  ...
  function _getRandomPower() internal view returns (uint256) {
    return
      uint256(keccak256(abi.encodePacked(blockhash(block.number - 30), block.timestamp, msg.sender, block.prevrandao)));
  }
}
